    ...
    <div className="min-h-screen pb-20">
      <header className="sticky top-0 z-40 border-b border-gray-200 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 sm:px-6 py-4">
          ...
          <Link href="/annonces" className="text-sm text-gray-600 hover:text-gray-900">← Toutes les annonces</Link>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 pt-8">
        ...
        <div className="mb-4 flex items-center gap-3 flex-wrap">
          ...
          {listing.sector && <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-500">{listing.sector}</span>}
          {listing.creation_date && <span className="text-xs text-gray-500">Créé en {listing.creation_date}</span>}
          {listing.views > 0 && <span className="text-xs text-gray-500">{listing.views} vues</span>}
        </div>
          ...
          <Section title="Photos">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {listing.images.map((url: string, i: number) => (
                ...
                <img key={i} src={url} alt={`${listing.title} - Photo ${i + 1}`} className="w-full h-48 object-cover rounded-xl border border-gray-200" />
              ))}
            </div>
          </Section>
        )}
        ...
        {listing.url && (
          <Section title="Site web">
            <a
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-6 py-3 text-sm text-emerald-400 hover:border-emerald-300 transition-all"
            >
              {listing.url}
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {listing.contact_email && (
              <a
                ...
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-6 py-3.5 text-sm font-semibold text-gray-900 hover:bg-emerald-600 transition-colors"
              >
            ...
            {listing.contact_whatsapp && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-6 py-3.5 text-sm font-semibold text-gray-900 hover:brightness-110 transition-all"
              >
